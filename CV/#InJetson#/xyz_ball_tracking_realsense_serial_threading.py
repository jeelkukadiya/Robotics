...
try:
    while True:
        ...
        gaussian_filter = cv2.cuda.createGaussianFilter(gpu_frame.type(), gpu_frame.type(), (11, 11), 0)

        # Apply the filter
        gpu_blurred = gaussian_filter.apply(gpu_frame)

        # Download result back to CPU memory if needed
        ...
        temp_mask_blue = cv2.inRange(hsv, blueLower, blueUpper)
        temp_mask_purple = cv2.inRange(hsv, purpleLower, purpleUpper)
        temp_mask_red = cv2.inRange(hsv, redLower, redUpper)
        
        # If further GPU processing is needed, upload masks back to GPU
        mask_blue = cv2.cuda_GpuMat()
        mask_blue.upload(temp_mask_blue)
        mask_purple = cv2.cuda_GpuMat()
        mask_purple.upload(temp_mask_purple)
        mask_red = cv2.cuda_GpuMat()
        mask_red.upload(temp_mask_red)
        ...
        for color, mask in [('blue', mask_blue), ('purple', mask_purple),('red', mask_red)]:
            # Define the kernel size, you can adjust it according to your needs
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))

            # Create morphology filters for erosion and dilation
            ...
            # Assuming 'gpu_mask_blue' is a cv2.cuda_GpuMat containing the thresholded mask
            mask = erode_filter.apply(mask)
            mask = dilate_filter.apply(mask)
            mask = mask.download()
            #mask = cv2.cuda.erode(mask, None, iterations=2)
            #mask = cv2.cuda.dilate(mask, None, iterations=2)
            mask = mask.download()
            if color == 'blue':

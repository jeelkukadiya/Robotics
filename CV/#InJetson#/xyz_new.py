...
def send_coordinates(x, y,verify, depth,ser):
    """ Send coordinates over serial """
    if ser is not None:
        try:
            message = f"{x},{y},{verify},{depth}\n"
            ser.write(message.encode())
            print(f"Sent coordinates: {x}, {y},verification is : {verify},Depth is:, {depth}")
        except Exception as e:
    ...
    while True:
        ...
        if nearest_ball_center and nearest_ball_depth > 0:
            ...
            with open("coordinates.txt", "w") as f:
                f.write(f"{nearest_ball_center[0]},{nearest_ball_center[1]}\n")
            # send_coordinates(nearest_ball_center[0], nearest_ball_center[1],color_verify,depth_in_mm, ser_port)

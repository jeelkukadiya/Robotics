...
def setup_serial(port=global_port, baudrate=9600, timeout=1,retry = 100 , delay = 2):
    ...
    while attempt < retry:
        try:
            ...
            attempt += 1
            time.sleep(delay)
    print(f"Failed to connect to {port} after {retry} attempts.")
    return None

def find_and_setup_serial(exclude_port=None,baudrate=9600,timeout=1,retry=100,delay=2):
    """ Find the port of the Arduino """
    ports = list(serial.tools.list_ports.comports())
    for port in ports:
        if exclude_port and port.device == exclude_port:
            continue
        ser = setup_serial(port.device,baudrate,timeout,retry,delay)
        if ser is not None:
            return ser
...
def send_coordinates(x, y,ser,retry=100,delay=2):
    ...
    if ser is not None:
        try:
            ...
            print(f"Failed to send data: {str(e)}.Attempting to reconnect...")
            new_ser = find_and_setup_serial(ser.port)
            if new_ser is not None:
                ...
                # try:
                #     #message = f"{x},{y}\n"
                #     new_ser.write(message.encode())
                #     print(f"Sent coordinates: {x}, {y}")
                # except Exception as e:
                #     print(f"Failed to send data: {str(e)}")
            else:
                print("Failed to restablish serial connection.")
    else:
...
ser_port = find_and_setup_serial(9600)
...
########################################################################
        ...
        # else:
            # print("No data received from Arduino")
########################################################################
        ser_port = send_coordinates(xx, yy, ser_port)
